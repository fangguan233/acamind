...
def run_chainlit(target: str):
    default_host = config.run.host if config and config.run else DEFAULT_HOST
    default_port = config.run.port if config and config.run else DEFAULT_PORT
    default_root = config.run.root_path if config and config.run else DEFAULT_ROOT_PATH
    host = os.environ.get("CHAINLIT_HOST", default_host)
    port = int(os.environ.get("CHAINLIT_PORT", default_port))
    root_path = os.environ.get("CHAINLIT_ROOT_PATH", default_root)

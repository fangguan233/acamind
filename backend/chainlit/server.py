...
def get_build_dir(local_target: str, packaged_target: str) -> str:
    ...
    repo_build_dir = os.path.join(repo_root, local_target, "dist")
    repo_marker = os.path.join(repo_root, "pnpm-workspace.yaml")
    ...
    elif os.path.exists(repo_marker) and os.path.exists(repo_build_dir):
...
class SafariWebSocketsCompatibleGZipMiddleware(GZipMiddleware):
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        no_gzip_prefixes = (
            f"{config.run.root_path}/tikzjax",
            f"{config.run.root_path}/assets/tikzjax",
        )

        # Do not gzip-compress already-compressed/binary assets (e.g., tikzjax *.gz resources).
        if path.endswith(".gz") or any(path.startswith(p) for p in no_gzip_prefixes):
            await self.app(scope, receive, send)
        # Prevent gzip compression for HTTP requests to socket.io path due to a bug in Safari
        elif path.startswith(SOCKET_IO_PATH):
            await self.app(scope, receive, send)
        else:
...
@router.get("/tikzjax/{filename:path}")
async def serve_tikzjax_file(
    filename: str,
):
    """Serve a file from tikzjax dir."""

    base_path = Path(os.path.join(build_dir, "tikzjax"))
    file_path = (base_path / filename).resolve()
...
async def get_logo(theme: Optional[Theme] = Query(Theme.light)):
    ...
    logo_path = None
    logo_candidates = [
        f"acamind_wordmark_{theme_value}",
        f"logo_{theme_value}",
    ]
    ...
    for base_dir in search_dirs:
        for candidate in logo_candidates:
            try:
                files = glob.glob(str(base_dir / f"{candidate}.*"))
            except Exception:
                files = []
            if files:
                logo_path = files[0]
    ...
    if not logo_path:
        for candidate in logo_candidates:
            files = glob.glob(os.path.join(build_dir, f"{candidate}.*"))
            if files:
                logo_path = files[0]
                break
    ...
    if not logo_path:
        for candidate in logo_candidates:
            files = glob.glob(os.path.join(build_dir, "assets", f"{candidate}*.*"))
            if files:
                logo_path = files[0]
                break
...
@router.get("/avatars/{avatar_id:path}")
async def get_avatar(avatar_id: str):
    ...
    avatar_id = avatar_id.replace("\x00", "").replace("/", " ").replace("\\", " ").strip()
    safe_avatar_id = re.sub(r"[^\w.-]+", "_", avatar_id, flags=re.UNICODE)
    safe_avatar_id = safe_avatar_id.strip("_.").lower()
    if not safe_avatar_id:
        return await get_favicon()

    base_path = Path(APP_ROOT) / "public" / "avatars"
    avatar_pattern = f"{safe_avatar_id}.*"

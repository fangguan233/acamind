...
def _normalize_thread_metadata(thread: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(chat_settings, dict) and not chat_settings:
        metadata.pop("chat_settings", None)
    elif chat_settings in (None, "", [], False):
        metadata.pop("chat_settings", None)
...
def _apply_thread_metadata_to_session(
    session: WebsocketSession, thread: Dict[str, Any]
) -> Dict[str, Any]:
    metadata = _normalize_thread_metadata(thread)
    existing = user_sessions.get(session.id) or {}
    user_sessions[session.id] = {**existing, **metadata.copy()}
...
def _restore_chat_context_from_thread(thread: Dict[str, Any]) -> None:
    chat_context.clear()
    for step in thread.get("steps", []):
        if not isinstance(step, dict):
            continue
        if "message" not in str(step.get("type") or ""):
            continue
...
async def resume_thread(session: WebsocketSession):
    ...
    if user_is_author:
        _apply_thread_metadata_to_session(session, thread)
        return thread
...
@sio.on("hydrate_local_thread")  # pyright: ignore [reportOptionalCall]
async def hydrate_local_thread(sid, thread: Dict[str, Any]):
    session = WebsocketSession.require(sid)
    context = init_ws_context(session)
    ...
    _apply_thread_metadata_to_session(session, thread)
    _restore_chat_context_from_thread(thread)
    ...
    if config.code.on_chat_resume:
        try:
            ...
            await context.emitter.emit(
                "first_interaction",
                {"interaction": "resume", "thread_id": session.thread_id},
            )
            session.has_first_interaction = True
        except Exception as e:
            logger.exception("Failed to hydrate local thread: %s", e)
            await context.emitter.send_toast("本地历史上下文恢复失败。", "error")

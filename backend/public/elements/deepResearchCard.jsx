      ...
      <CardHeader className="pb-3">
        ...
        >
          <div className="space-y-1">
            <CardTitle className="text-base">第 {round.round} 轮</CardTitle>
            <div className="text-xs text-muted-foreground">
              {round.status === 'in_progress' ? '进行中' : round.status === 'failed' ? '失败' : '已记录'}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {toolRuns.length ? (
                toolRuns.map((item, index) => (
                  <div key={`${item.tool_name || 'tool'}-${index}`} className="rounded-md border p-3 text-sm">
                    <div className="font-medium">{item.tool_name || 'tool'}</div>
                    <div className="mt-1 text-muted-foreground">{item.summary || '暂无摘要'}</div>
                  </div>
                ))
              ) : (
                <div className="rounded-md bg-slate-50 p-3 text-sm text-muted-foreground">暂无</div>
              )}
            </div>
          </div>
          <div className="space-y-2">
  ...
  return (
    <Card className="border-amber-200 bg-amber-50/60 shadow-none">
      <CardHeader className="pb-3">
        ...
        <div className="text-sm text-muted-foreground">{request.reason || '请先明确研究方向'}</div>
      </CardHeader>
      <CardContent className="space-y-4">
            ...
            <div key={question.id} className="space-y-2">
              ...
              <div className="space-y-2">
                {options.map((option) => {
                  ...
                  return (
                    <label key={option.label} className="flex cursor-pointer items-start gap-2 rounded-md border p-3 text-sm">
                      <input
                        type="radio"
                        name={question.id}
                        checked={checked}
                ...
                {question.allow_other ? (
                  <Input
                    ...
                    onChange={(event) => updateAnswer(question.id, event.target.value, 'other')}
                  />
                ) : null}
              </div>
            </div>
    ...
    <Card className="border-dashed border-slate-300 shadow-none">
      <CardContent className="space-y-3 pt-6">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline">待补原文</Badge>
          <div className="text-sm font-medium">{request.title || '未命名论文'}</div>
        </div>
        <div className="text-sm text-muted-foreground">{request.reason || '需要用户补充 PDF 才能继续精读。'}</div>
        {Array.isArray(request.source_links) && request.source_links.length ? (
          <div className="space-y-1 text-sm">
            {request.source_links.map((item, index) => (
              <a
                key={`${item}-${index}`}
...
function DeepResearchCard() {
  ...
  const clarificationRequests = Array.isArray(card.clarification_requests) ? card.clarification_requests : [];
      ...
      <Card className="shadow-none">
        <CardHeader className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Badge variant={statusTone(summary.status)}>{summary.status_label || summary.status || '未知'}</Badge>
          </div>
      ...
      {clarificationRequests.length ? (
        <div className="space-y-3">
          {clarificationRequests.map((request) => (
            <ClarificationCard key={request.request_id || request.id} request={request} />
          ))}
        </div>
      ) : null}

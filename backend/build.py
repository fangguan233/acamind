...
def pnpm_install(project_root: pathlib.Path, pnpm_cmd: list[str]):
    run_subprocess([*pnpm_cmd, "install", "--frozen-lockfile"], project_root)


def pnpm_buildui(project_root: pathlib.Path, pnpm_cmd: list[str]):
    run_subprocess([*pnpm_cmd, "buildUi"], project_root)


def pnpm_build_react_client(project_root: pathlib.Path, pnpm_cmd: list[str]):
    run_subprocess([*pnpm_cmd, "-C", "libs/react-client", "run", "build"], project_root)


def pnpm_build_copilot(project_root: pathlib.Path, pnpm_cmd: list[str]):
    run_subprocess([*pnpm_cmd, "-C", "libs/copilot", "run", "build"], project_root)


def pnpm_build_frontend(project_root: pathlib.Path, pnpm_cmd: list[str]):
    run_subprocess([*pnpm_cmd, "-C", "frontend", "run", "build"], project_root)
...
def build():
    ...
    if os.environ.get("CHAINLIT_SKIP_UI_BUILD", "").strip().lower() in {"1", "true", "yes"}:
        print("-- Skipping UI build (CHAINLIT_SKIP_UI_BUILD=1)")
        return
    ...
    try:
        ...
        if pnpm_path:
            ...
            if corepack:
                print("-- pnpm not found, using corepack shim")
                pnpm_cmd = [corepack, "pnpm"]
        if not pnpm_cmd:
            raise BuildError("pnpm not found!")

        pnpm_install(project_root, pnpm_cmd)
        # Avoid calling the root "buildUi" script because it shells out to `pnpm`
        # again, which fails on machines relying on `corepack pnpm` without a pnpm shim.
        pnpm_build_react_client(project_root, pnpm_cmd)
        pnpm_build_copilot(project_root, pnpm_cmd)
        pnpm_build_frontend(project_root, pnpm_cmd)

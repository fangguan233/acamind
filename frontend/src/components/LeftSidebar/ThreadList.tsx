...
interface ThreadListProps {
  ...
  error?: string;
  hasHydratedInitialHistory: boolean;
  isFetching: boolean;
...
export function ThreadList({
  ...
  const handleDeleteThread = async () => {
    ...
    if (localHistoryEnabled) {
      const history = deleteLocalThread(threadIdToDelete);
  ...
  const handleRenameThread = () => {
    ...
    if (localHistoryEnabled) {
      const history = renameLocalThread(threadIdToRename, threadNewName);
          ...
          return (
            <SidebarGroup key={group}>
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {items.map((thread) => {
                    const selectedThreadId = currentThreadId || idToResume;
                    const isResumed = idToResume === thread.id && !currentThreadId;
                  ...
                  onShare={
                                    dataPersistence && threadSharingReady && !localHistoryEnabled

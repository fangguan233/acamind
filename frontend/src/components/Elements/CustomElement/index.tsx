...
import BuiltinDeepResearchCard from './BuiltinDeepResearchCard';
...
const CustomElement = memo(function ({ element }: { element: ICustomElement }) {
  ...
  const isBuiltinDeepResearchCard = element.name === 'deepResearchCard';
  const buildEndpoint = useCallback(
    (path: string) => apiClient.buildEndpoint(path),
    [apiClient]
  );

  useEffect(() => {
    if (isBuiltinDeepResearchCard) {
      return;
    }
    apiClient
  ...
  }, [isBuiltinDeepResearchCard, element.name, apiClient]);
  ...
  const sendMessagePayload = useCallback(
    (payload: Record<string, unknown>, fileRefs?: Array<{ id: string }>) => {
      return sendMessage(payload as any, fileRefs as any);
    },
    [sendMessage]
  );

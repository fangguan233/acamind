...
export const chatSettingsValueState = atom<Record<string, any>>({
  ...
  effects: [localStorageEffect<Record<string, any>>('chainlit_chat_settings_v1')]
});
    ...
    ({ setSelf, onSet }: { setSelf: any; onSet: any }) => {
      onSet(
        (
          ...
          if (
            newValue?.threads &&
            !isEqual(newValue.threads, oldValue?.threads)
          ) {
...
export type LatestOutputSnapshot = {
  mode: string;
  message_id?: string;
  content_preview?: string;
  regen_payload?: Record<string, any>;
  created_at?: number;
};

export const latestOutputsState = atom<Record<string, LatestOutputSnapshot>>({
  key: 'LatestOutputs',
  default: {}
});

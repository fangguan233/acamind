...
const useChatInteract = () => {
  ...
  const resetLatestOutputs = useResetRecoilState(latestOutputsState);
  ...
  const clear = useCallback((options?: { preserveSettings?: boolean }) => {
  ...
  const regenLatestOutput = useCallback(
    (snapshot: Record<string, any>) => {
      session?.socket.emit('window_message', {
        type: 'regen_latest_output',
        snapshot
      });
    },
    [session?.socket]
